...
find . -not -path './.*' -type f -exec sed -e "s,cosmosregistry/example,$MODULE_NAME,g" {} \;
find . -name '*.proto' -type f -exec sed -e "s,cosmosregistry.example,$(echo "$MODULE_NAME" | tr '/' '.'),g" {} \;
find . -name 'protocgen.sh' -type f -exec sed -e "s,rm -rf github.com cosmosregistry,rm -rf github.com $USERNAME,g" {} \;
find . -not -path './.*' -type f -exec sed -e "s,example,$REPO,g" {} \;
